...
/**
 * 作者:王浩 邮件:bingoogolapple@gmail.com 创建时间:15/7/6 下午10:13 描述:
 */
public class BGABadgeViewHelper {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (mDragable && mIsShowBadge
                        && mBadgeRectF.contains(event.getX(), event.getY())) {
                    mIsDraging = true;
                    ...
                    mDropBadgeView.setStickCenter(
                            badgeableRect.left + mBadgeRectF.left
                                    + mBadgeRectF.width() / 2,
                            badgeableRect.top + mBadgeRectF.top
                                    + mBadgeRectF.height() / 2);
    ...
    private void drawDrawableBadge(Canvas canvas) {
        mBadgeRectF.left = mBadgeable.getWidth() - mBadgeHorizontalMargin
                - mBitmap.getWidth();
        ...
        switch (mBadgeGravity) {
            ...
            case RightCenter:
                ...
                break;
            case RightBottom:
                mBadgeRectF.top = mBadgeable.getHeight() - mBitmap.getHeight()
                        - mBadgeVerticalMargin;
                break;
            default:
                break;
        }
        canvas.drawBitmap(mBitmap, mBadgeRectF.left, mBadgeRectF.top,
                mBadgePaint);
    ...
    private void drawTextBadge(Canvas canvas) {
        ...
        // 绘制徽章背景
        canvas.drawRoundRect(mBadgeRectF, badgeHeight / 2, badgeHeight / 2,
                mBadgePaint);
